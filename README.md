# 🛍️ Fashionable — E-Commerce Price Comparator & Growth Platform

[![Live Demo](https://img.shields.io/badge/Live%20Demo-Vercel-000000?style=for-the-badge&logo=vercel)](https://fashionable-fullstack-web.vercel.app)
[![Website](https://img.shields.io/badge/Official%20Site-getfashionable.shop-FF4081?style=for-the-badge&logo=google-chrome)](https://getfashionable.shop)
[![Tech Stack](https://img.shields.io/badge/Stack-Next.js%20%7C%20PocketBase%20%7C%20Railway-007ACC?style=for-the-badge)](https://github.com/devanshkrishna05-ui/fashionable-fullstack)
[![Growth Engine](https://img.shields.io/badge/Growth-Meta%20Ads%20%7C%20GA4%20%7C%20SEO-FF6F00?style=for-the-badge)]()

> **Fashionable** is a full-stack e-commerce price comparison platform designed to eliminate overpaying across major fashion platforms. Built with an integrated data-driven growth marketing, analytics, and performance acquisition engine.

---

## 🌟 Key Product Features

- 🔍 **Real-Time Price Comparison**: Instantly compare fashion product prices across top Indian e-commerce aggregators.
- 🔔 **Smart Price Drop Alerts**: Passwordless OTP authentication for instant email price reduction notifications.
- ⚡ **Automated Search Index**: High-speed catalog search and indexing engine.
- 📱 **Mobile-First Responsive UX**: High conversion rate (CRO) visual interface.

---

## 📊 Growth Marketing & Data Analytics Engine

Engineered with a data-first approach for customer acquisition, attribution, and retention:

### 1. 📈 Performance Marketing & Paid UA (Meta & Google Ads)
- **Meta Ads Architecture**: Custom CBO/ABO campaign structures targeting high-intent fashion shoppers (Zara, Myntra, Ajio interest stacks).
- **Google Search & PMax**: Performance Max campaigns targeting high-intent transactional search terms.
- **CAC & ROAS Optimization**: A/B testing ad creatives to minimize Cost Per Acquisition (CAC) and maximize Return On Ad Spend.

### 2. 🔍 SEO & Conversion Rate Optimization (CRO)
- Structured schema metadata for search engines to capture high organic buyer traffic.
- High-converting funnel design reducing bounce rates on product comparison pages.

### 3. 🎯 Analytics & Tracking (GA4 & Event Attribution)
- Event tracking across the user journey (Search -> Compare -> Price Alert Signup -> Outbound Merchant Click).
- Funnel analysis to measure user retention and engagement loops.

---

## 🛠️ Architecture & Tech Stack

| Layer | Technology |
| :--- | :--- |
| **Frontend** | Next.js (TypeScript), Tailwind CSS, Modern UI Components |
| **Backend & DB** | PocketBase (Hosted on Railway) |
| **Deployment** | Vercel (Frontend) & Railway (Backend) |
| **Monorepo** | Turborepo / npm workspaces |

---

## 🚀 Live Links

- 🌐 **Web Application**: [fashionable-fullstack-web.vercel.app](https://fashionable-fullstack-web.vercel.app)
- 🛍️ **Official Domain**: [getfashionable.shop](https://getfashionable.shop)

---
*Created & Maintained by [Devansh Krishna](https://github.com/devanshkrishna05-ui)*
